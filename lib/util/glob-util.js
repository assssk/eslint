...
function listFilesToProcess(globPatterns, options) {
     ...
     * @param {string} pattern The pattern which matched this file.
     * @returns {void}
     */
    function addFile(filename, shouldWarnIgnored, ignoredPaths, pattern) {
        let ignored = false;
        ...
        if (added[filename]) {
            added[filename].patterns.push(pattern);
            return;
        }
        added[filename] = {filename, ignored, patterns: [pattern]};
        files.push(added[filename]);
    }
    ...
    globPatterns.forEach(function(pattern) {
        ...
        if (shell.test("-f", file)) {
            ...
            addFile(fs.realpathSync(file), !shell.test("-d", file), ignoredPaths, pattern);
        } else {
            ...
            new GlobSync(pattern, globOptions, shouldIgnore).found.forEach(function(globMatch) {
                addFile(path.resolve(cwd, globMatch), false, ignoredPaths, pattern);
            });
        }
    });
