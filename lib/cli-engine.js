...
CLIEngine.prototype = {
    ...
    executeOnFiles(patterns) {
        ...
        const resolvedPatterns = this.resolveFileGlobPatterns(patterns);
        const fileList = globUtil.listFilesToProcess(resolvedPatterns, options);
        const reportedResolvedPatterns = new Set();

        // Does lint
        fileList.forEach(function(fileInfo) {
            fileInfo.patterns.forEach(pattern => {
                reportedResolvedPatterns.add(pattern);
            });
            executeOnFile(fileInfo.filename, fileInfo.ignored);
        });

        // Checks patterns which match no file.
        resolvedPatterns.forEach((resolvedPattern, index) => {
            if (!reportedResolvedPatterns.has(resolvedPattern)) {
